...
- 옵션으로 refetchInterval 을 초단위 지정하면 `stale` 상태면 `Refetching` 으로 데이터 호출함.
...
export function useTodoDataById(id: number) {
  return useQuery({
    ...
    refetchOnReconnect: false, // 재연결 시점에 리패칭 끄기
